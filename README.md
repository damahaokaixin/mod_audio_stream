...
This module supports raw binary audio streaming over WebSocket, and can optionally play audio returned by the WebSocket server. Playback runs independently from forward streaming, enabling full-duplex audio between the caller and the WebSocket endpoint.

Key features:

- Full-duplex audio streaming (caller ↔ WebSocket)
- Upstream (FreeSWITCH → WebSocket) uses raw binary audio frames
- Downstream playback (WebSocket → caller) supports base64-encoded audio payloads
- Playback can be tracked, paused, and resumed dynamically

Notes:
- This source tree is MIT-licensed and does not implement a hard concurrent-channel limit.
...
- Heart beat value must be a positive integer. Non-positive values are ignored.
...
- `sampling-rate` - choice of
  ...
  - or an integer multiple of 8000 (e.g. 24000). Invalid values are rejected.
...
Event generated by the module (subclass: _mod_audio_stream::play_) will be the same as the `data` element with the **file** added to it representing filePath. The original `audioData` is replaced with `"<omitted>"` to avoid huge event/log payloads:
```json
{
  "audioDataType": "raw",
  "sampleRate": 8000,
...
```
If printing to the log is not suppressed, `response` printed to the console will look the same as the event.

Safety limits:
- `audioData` is rejected if it is too large (decoded payload > 10MB)
- per session, at most 100 playback files can be generated
